...
<div class="relative flex flex-col mx-auto w-full text-sm sm:text-base min-h-screen">
	...
	>
		<button
			on:click={goTop}
			class="ml-auto rounded-full aspect-square bg-slate-900 text-violet-400 px-3 sm:px-4 hover:bg-slate800 cursor-pointer"
		>
			<i class="fa-solid fa-arrow-up grid place-items-center aspect-square" />
		</button>
	</div>
	<Header {y} />
	<div class="max-w-[1400px] translate mx-auto"><slot /></div>
